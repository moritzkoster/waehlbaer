...
from Wählbär import DAYS, SLOTS_PER_DAY, Allocation, Block, MetaBlock, Schedule, Unit
...
def export_to_pdf(unit):
        ...
        for time in range(SLOTS_PER_DAY):
            ...
            if len(block) == 1:
                ...
                for i, block in enumerate(block):
                    print("  -  ", block.ID)
    group_map = {"pf": "Pfadis", "pi": "Pios", "wo": "Wölfe"}
    ...
    for entry in unit.schedule.get_list(with_slot=True):
        ...
        for slot in [slot, *next_N_slots]:
            ...
            if (
                unit.group == "wo" and unit.present_on[0] >= 3
            ):  # wölfe have a different template for days 3-12
                tab -= 2
            ...
            shading_elm.set(
                qn("w:fill"), color_map.get(entry["element"].data["cat"], "#808080")
            )  # default white color
...
def export_block_to_pdf(block):
    ...
    for entry in block.schedule.get_list(with_slot=True):
        ...
        for slot in [slot, *next_N_slots]:
            ...
            shading_elm = OxmlElement("w:shd")
            shading_elm.set(
                qn("w:fill"), group_colors.get(entry["element"].group, "#808080")
            )  # default white color
            cell._tc.get_or_add_tcPr().append(shading_elm)
...
def NA_map(block_ID):
    id = ord(block_ID[-1]) - 65
    print(block_ID)
    map = ["4a", "4b", "NA 2", "5", "8a", "8b", "8c", "8d", "NA 1", "NA 3"]
    return "ON-05 " + map[id]
...
def load_unitlist(
    allocation,
    path="data",
    filename="Antworten Buchungstool.xlsx",
    print_enabled=False,
    ignore_warnings=False,
):
    full_labels = pd.read_excel(
        os.path.join(path, filename), sheet_name="Formularantworten 1", header=1
    ).columns.tolist()
    df = pd.read_excel(
        os.path.join(path, filename), sheet_name="Formularantworten 1", header=2
    )
    ...
    for ic, col in enumerate(df.columns):
        ...
        if col.startswith("Unnamed") and full_labels[ic].startswith("Unnamed"):
            if print_enabled:
                print(
                    f"\033[31m{new:>20}\033[0m: {display_full_label} (will get removed)"
                )
            ...
            try:
                ...
                if print_enabled:
                    print(f"\033[32m{new:>20}\033[0m: {display_full_label}")
            except:
                if print_enabled:
                    print(
                        f"\033[31m{new:>20}\033[0m: '{display_full_label}' will get removed for no parsing"
                    )
                new = full_labels[ic]
        elif not col.startswith("Unnamed") and not full_labels[ic].startswith(
            "Unnamed"
        ):
            if print_enabled:
                print(f"\033[32m{new:>20}\033[0m: {display_full_label}")
            ...
            print(
                f"\033[31mERROR Label with no fullname \033[0m: '{display_full_label}' will get removed"
            )
            # new = full_labels[ic]
    ...
    for group in ["Pios", "Pfadis", "Wölfe"]:
        ...
        df_group = df_group[
            [
                c
                for c in df.columns
                if f"_{group[:2].lower().replace('ö', 'o')}_" in c or "_all" in c
            ]
        ]
    ...
    tn_numbers = pd.read_excel(
        os.path.join(path, "EH_Übersicht_Einheiten.xlsx"),
        sheet_name="Übersicht_Einheiten",
        header=1,
    )
    non_empty_count = tn_numbers.iloc[:, 0].count() + 1
    tn_numbers = tn_numbers.head(non_empty_count)
    tn_numbers = tn_numbers[
        [
            "ID",
    ...
    for df_ in [df_pi, df_pf, df_wo]:
        for i in range(df_.shape[0]):
            ...
            if tn_numbers.loc[ID, "Datum"] == "12.-25. Juli 2026":
                data["present_on"] = [
                    e - 12 for e in range(12, 25 + 1)
                ]  # -12 to convert to wählbär day (12.7. is Day 0)
            if tn_numbers.loc[ID, "Datum"] == "13.-18. Juli 2026":
                data["present_on"] = [
                    e - 12 for e in range(14, 17 + 1)
                ]  # 14 for no program on day 13
            if tn_numbers.loc[ID, "Datum"] == "20.-25. Juli 2026":
                data["present_on"] = [e - 12 for e in range(21, 25 + 1)]

            allocation.append_unit(Unit(ID, data), ignore_warnings=ignore_warnings)

    for row in tn_numbers.itertuples():
        ID = row.Index
        if all([ID != unit.ID for unit in allocation.UNITS]):
            print(
                f"{FORMAT.RED}WARNING: Unit {ID} is not in booking tool responses ({row.Betreuungsperson}).{FORMAT.RESET}"
            )
...
def load_blocklist(allocation, path="data", filename="PRG_Blockliste.xlsx"):
    ...
    df_offsite = pd.read_excel(
        os.path.join(path, filename), sheet_name="Off Site & Wasser"
    )
    ...
    for bd in df.itertuples():
        ...
        if not pd.isna(bd.mix_units):
            if (
                "2/3 Einheiten zusammen" in bd.mix_units
                or "ganzes KALA" in bd.mix_units
            ):
                mix_units = True
        if bd.verteilungsprio and pd.notna(bd.verteilungsprio):
            allocation.vp_bonus[bd.ID] = (
                5 - int(str(bd.verteilungsprio)[0])
            ) / 5  # higher prio -> higher bonus, max bonus is 0.8 for prio 1
...
def write_to_xlsx(allocation, fname="allocation.xlsx", path="saves", comment=""):
    ...
    for iu, unit in enumerate(allocation.UNITS):
        ...
        worksheet.merge_range(
            "B1:O1", f"{unit.ID}: {unit.fullname} ({unit.group})", merge_format
        )
        ...
        for blocks in unit.schedule.get_time_list():
            worksheet.write(
                slot_to_xlsx_cell(blocks["slot"]), "/".join(blocks["elements"])
            )

        row = 10
        col = 0

        worksheet.write(f"{chr(col + 65)}{row}", "info")
        row += 1
        col += 1
        worksheet.write(f"{chr(col + 65)}{row}", f"n_people: {unit.n_people}")
        col += 1

        row += 1
        col = 0
        worksheet.write(f"{chr(col + 65)}{row}", "general")
        row += 1
        col += 1
        for key, value in unit.general.items():
            ...
            col += 1
            worksheet.write(f"{chr(col + 65)}{row}", f"{value}")
            col += 1
        ...
        for cat, prios in unit.prios_sorted.items():
            ...
            for prio in prios:
                ...
                if prio["value"] >= 2 and unit.has_block(prio["ID"]):
                    ...
                    worksheet.write(
                        f"{chr(col + 65)}{row}", f"{prio['ID']}: {prio['value']}"
                    )
                col += 1
            row += 1
    ...
    for ib, block in enumerate(allocation.BLOCKS):
        ...
        worksheet.merge_range(
            "B1:O1",
            f"{block.ID}: {block.data['fullname']} ({block.data['cat']})",
            merge_format,
        )
        for i in range(1, SLOTS_PER_DAY + 1):
            worksheet.write(f"A{i + 2}", f"slot {i}")
        ...
        for units in block.schedule.get_time_list():
            worksheet.write(
                slot_to_xlsx_cell(units["slot"]), "/".join(units["elements"])
            )
...
def read_from_xlsx(a, path="saves", filename="allocation.xlsx", unit_truth=True):
    ...
    for unit_n in unit_names:
        ...
        if not a.get_unit_by_ID(unit_n):
            print(
                f"{FORMAT.RED}WARNING: Unit {unit_n} in xlsx not found in allocation.{FORMAT.RESET}"
            )
    ...
    for block_n in block_names:
        if not a.get_block_by_ID(block_n):
            print(
                f"{FORMAT.RED}WARNING: Block {block_n} in xlsx not found in allocation.{FORMAT.RESET}"
            )
            if input("create? [y/n]: ").lower() == "y":
                a.append_block(
                    Block(
    ...
    for block_n in block_names:
        ...
        for item in get_list_from(df):
            ...
            if (
                block not in unit.schedule[item["slot"]]
            ):  # test if block is actually in unit schedule for this slot, otherwise print warning and add it
                print(
    ...
    for unit_n in unit_names:
        print(
            f"\rTesting if unit {unit_n:>4} has correct schedule...", end="", flush=True
        )
        df = get_df_from_sheet_name(workbook, unit_n)
...
def export_TN_overwiew_to_xlsx(allocation, fname="TN_Overview.xlsx", path="exports"):
    ...
    for i in range(1, DAYS):
        ...
        worksheet.merge_range(
            f"{column_number_to_letter(start)}2:{column_number_to_letter(stop)}2",
            f"{i + 12}.07.",
        )

        for j in range(SLOTS_PER_DAY):
            worksheet.write(
                f"{column_number_to_letter(start + j)}3",
                ["VM", "NM1", "NM2", "AB", "NA"][j],
            )

            # set column with to 0.8
            worksheet.set_column(
                f"{column_number_to_letter(start + j)}:{column_number_to_letter(start + j)}",
                3,
            )
    ...
    for block in allocation.BLOCKS:
        ...
        for day in range(1, DAYS):
            for time in range(SLOTS_PER_DAY):
                ...
                worksheet.write(
                    f"{col}{row}",
                    ", ".join([unit.ID for unit in block.schedule[slot]]),
                    fmt,
                )
    ...
    for day in range(DAYS):
        for time in range(SLOTS_PER_DAY):
            ...
            for e in value.split("/"):  # allow multiple blocks per slot
                if e.strip():
                    result.append(
                        {"slot": Schedule.idx2str(day, time), "element": e.strip()}
                    )
    return result
