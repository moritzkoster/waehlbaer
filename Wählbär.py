...
class Schedule:
        ...
        for idd, day in enumerate(self.calendar):
            for it, time in enumerate(day):
                if time:
                    l.append(
                        {
                            "slot": self.idx2str(idd, it),
                            "elements": [e.ID for e in time],
                        }
                    )
        return l
    ...
    def remove_entry(self, entry=None, slot=None):
        if type(entry) != Block and type(entry) != Unit:
            print(
                f"ERROR: entry type ({type(entry)}) is wrong, expected 'Block()' or 'Unit()'"
            )
            return 0
        if not entry and not slot:
            print("ERROR: bro what?? remove what?")
            return 0
            ...
            for idd, day in enumerate(self.calendar):
                for itt, time in enumerate(day):
                    if entry in time:
                        ...
                        return 0
            print(f"ERROR: couldnt find entry with ID {entry.ID}  in schedule")
            return 0
        ...
        if slot:
            if len(self[slot]) == 1:
                ...
                return 0
            if len(self[slot]) == 0:
                print("ERROR: slot already empty")
                return 0
            print(f"ERROR: more than one block in slot. Be more specific")
            return 0

    def remove_block(self, block=None, slot=None):
        if type(self.owner) == Block:
            print("ERROR: cannot remove block from Block()")
        if not block:
            block = self[slot][0]
    ...
    def remove_unit(self, unit=None, slot=None):
        if type(self.owner) == Unit:
            print("ERROR: cannot remove unit from Unit()")
        if not unit:
            unit = self[slot][0]
            ...
            for iss, slot in enumerate(day):
                if type(self.owner) == Block:
                    if self.owner.get_space((idd, iss)) > 0:
                        slot_list.append(
                            {
                                "slot": self.idx2str(idd, iss),
                                "space": self.owner.get_space((idd, iss)),
                            }
                        )
                else:
                    if slot == []:
    ...
    def matching_slots(unit_slots, block_slots):
        if type(block_slots) == list:
            ...
            print(
                f"{RED}{BOLD}: unit_slots must be 'list' and block_slots must be 'list' or dict of lists{RESET}"
            )
...
def max_per_week(slot, self, block_req):
    if block_req["cat"] not in ["wald", "nacht", "dusche", "flussbaden"]:
        return True
    ...
    max_counts_per_week = {"wald": 3, "nacht": 2, "dusche": 1, "flussbaden": 1}

    return count < max_counts_per_week[block_req["cat"]]


def max_per_day(slot, self, block_req):
    if block_req["cat"] not in ["wald"]:
        return True
    count = 0
    for time in self.schedule.calendar[Schedule.to_idx(slot)[0]]:
        for block in time:
...
def long_blocks(slot, self, blockdata):
    ...
    for i, slot in enumerate(next_slots):
        if self.schedule[slot]:
            if (
                blockdata["ID"]
                in ["OFF-17", "OFF-18", "OFF-19", "OFF-20", "OFF-24", "OFF-25"]
                and self.schedule[slot][0].ID == "ON-39"
            ):  # OVERRIDE zweitageswanderung darf während freizeit sein
                continue
            if (
                blockdata["cat"] == "ausflug" and self.schedule[slot][0].ID == "ON-39"
            ):  # override ausflüge wärend freizeit
                continue
            return False
    return True
...
def on_times_block(slot, self, unit_req):
    return (
        False
        if "on_times" in self.data
        and Schedule.to_idx(slot)[1] not in self.data["on_times"]
        else True
    )
...
def on_days_block(slot, self, unit_req):
    return (
        False
        if "on_days" in self.data
        and Schedule.to_idx(slot)[0] not in self.data["on_days"]
        else True
    )
...
def not_in_slot_block(slot, self, unit_req):
    return (
        False
        if "not_in_slot" in self.data and slot in self.data["not_in_slot"]
        else True
    )
...
def on_days_unit(slot, self, unit_req):
    return (
        False
        if "on_days" in unit_req and Schedule.to_idx(slot)[0] not in unit_req["on_days"]
        else True
    )
...
def on_times_unit(slot, self, unit_req):
    return (
        False
        if "on_times" in unit_req
        and Schedule.to_idx(slot)[1] not in unit_req["on_times"]
        else True
    )
...
class Block:
    ...
    def get_group_space(self, slot):
        if not self.schedule[slot]:
            if "hard_limit" in self.data and self.data["hard_limit"] == "Ja":
                ...
                return (
                    99,
                    ["wo", "pf", "pi"],
                )  # returns 99 for first units, so that is only relevant when there are already units in the block
        ...
        for unit in self.schedule[slot]:
            ...
            if unit.group != group:
                print(
                    f"{RED}ERROR: two different groups assigned to block {self.ID}{RESET}"
                )
    ...
    def set_unit(self, unit, slot):
        if type(slot) == dict:
            ...
            print("ERROR: slot must be str 'A0' or dict {'slot': 'A0', ...}")
            return
        ...
        for iss, slot in enumerate(free_slots):
            ...
            for rule in self.rules:
                if not rule(slot["slot"], self, requirements):
                    matching = False
                    if return_reason:
                        search_result.reason.add(
                            f"Block-Rule '{rule.__name__}' not fulfilled"
                        )
                    break
            if matching:
    ...
    def to_dict(self):
        ...
        d = {"type": "block", "ID": self.ID, "data": self.data, "schedule": unit_list}
        return d
...
class MetaBlock(Block):
    ...
    def add_subblock(self, block):
        if type(block) != Block:
            print(f"ERROR: type is not block but '{type(block)}'")
            return 0
    ...
    def set_unit(self, unit, slot):
        if type(slot) != dict:
            print("ERROR: MetaBlock expects slot as dict of '{ID:OFF-12: slot:A1}'")
            return
        for sub_block in self.sub_blocks:
            if sub_block.ID == slot["ID"]:
                ...
                return
        print(
            f"ERROR: could not find sub_block with ID '{slot['ID']}' in MetaBlock '{self.ID}'"
        )
...
class Unit:
    def __init__(self, ID, data):
        ...
        del data["email"]
        self.group = data["group"].lower().replace("ö", "o")
        del data["group"]
        self.more_or_less = data["more_or_less"]
        del data["more_or_less"]
        self.wasser_anerk = data["wasser_anerk"]
        del data["wasser_anerk"]
        self.present_on = data["present_on"]
        del data["present_on"]
    ...
    def score_sum_norm(self):  # TODO
        if not hasattr(self, "norm_const"):
            self.norm_const = np.array(
                [NRANKS - p["rank"] for p in self.data["prios"]]
            ).sum()
            ...
            for iss, slot in enumerate(self.schedule.free_slots()):
                ...
                for rule in self.rules:
                    if not rule(slot, self, block_req):
                        ...
                        if return_reason:
                            search_result.reason.add(
                                f"Unit-rule '{rule.__name__}' not fulfilled"
                            )
                        break
                if matching:
                    slots.append(slot)
    ...
    def highest_unmatched_prios(self, N=5):  # TODO
        print(
            "\033[31m'highest_unmatched_prios()' IS DEPRECATED, USE 'get_highest_unmatched_by_cat()' INSTEAD\033[0m]"
        )
    ...
    def sample_top_N_prios(self, N):
        print(
            "\033[31m'sample_top_N_prios()' IS DEPRECATED, NO REPLACEMENT SO FAR\033[0m]"
        )
        prios = self.get_unmatched_prios()
        if len(prios) >= N:
    ...
    def sort_prios_by_cat(self):
        if not hasattr(self.allocation, "block_cats"):
            print("\033[1m\033[31mERR: call 'find_block_cats()' first\033[0m")
        prios = {}
        general = {}
        wish = 0
        total = 0
...
class Allocation:
    ...
    def load(self, fname, path="saves"):
        if not self.UNITS or not self.BLOCKS:
            print(
                f"{RED}{BOLD}Load units and blocklists first with 'load_unitlist(a)' and 'load_blocklist(a)'{RESET}"
            )
    ...
    def append_block(self, block, index=None):
        if type(block) != Block and type(block) != MetaBlock:
            print(f"ERROR: type is not block but '{type(block)}'")
            return 0
    ...
    def append_unit(self, unit, ignore_warnings=False):
        if type(unit) != Unit:
            print(f"ERROR: type is not block but '{type(unit)}'")
            return 0
        if self.get_unit_by_ID(unit.ID, print_error=False):
            print(
                f"{YELLOW}WARN: unit with ID '{unit.ID}' already exists and is replaced{RESET}"
            )
    ...
    def get_verteilungsprio_block(self, p=1):
        ...
        for block in self.BLOCKS:
            if (
                "verteilungsprio" in block.data
                and block.data["verteilungsprio"]
                and str(p) in str(block.data["verteilungsprio"])
            ):
                blocks.append(block)
        return blocks
                ...
                Unit(
                    ...
                    data={
                        ...
                        "prios": [
                            {
                                "ID": "block" + str(ii + 1),
                                "rank": min(5, random.randint(1, 20)),
                            }
    ...
    def print_stats(self):
        ...
        for unit in self.UNITS:
            print(
                f"{BLUE}{unit.ID:3}{RESET}: score={unit.score():6.4f}, blocks={len(unit.schedule.get_list()):3.0f}, tags={YELLOW}{', '.join(unit.tags)}{RESET}"
            )
        print(
            f"Scores: sum={s.sum():6.4f}, min={s.min():5.4f}, max={s.max():5.4f}, std={s.std():5.4f}"
        )
        print(
            f"Blocks assigned: min={b.min():3.0f}, max={b.max():3.0f}, std={b.std():5.2f}"
        )
    ...
    def log_stats(self, path, runtime):
        ...
        with open(path, "a") as file:
            ...
            file.write(
                f"{s.sum():6.4f}, {s.min():5.4f}, {s.max():5.4f}, {s.std():5.4f}, "
            )
